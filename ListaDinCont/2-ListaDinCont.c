...
struct lista {
    int qtd;
    struct ED_LE dados[9];
};

Lista* cria_lista(int max) {
    Lista *li = (Lista*)malloc(max * sizeof(struct lista));
    if (li != NULL) {
        li->qtd = 0;
    }
    return li;
...
int insere_lista_final(Lista* li, struct ED_LE edle, int max) {
    if (li == NULL) {
        return 0;
    }
...
int insere_lista_inicio(Lista* li, struct ED_LE edle, int max) {
    if (li == NULL) {
        return 0;
    }
    if (lista_cheia(li, max)) {
...
int insere_lista_ordenada(Lista* li, struct ED_LE edle, int max) {
    if (li == NULL) {
        return 0;
